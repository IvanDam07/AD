...
public class Departamento {
    ...
    //insertar pero con un result set
...
//            }
    ...
    public static int llamarFNEmpleado(OperacionesBBDD bbdd, int dept_no) {
        int numEmpleados = 0;
        try {
            String sql = "{? = call f_n_empleado(?)}";
    ...
    public static void mostrarInformacionTabla(OperacionesBBDD bbdd, String nombreTabla) {
        try {
            // Obtener la clave primaria
            String sqlPK = "SELECT column_name FROM all_cons_columns "
                    + "WHERE constraint_name = (SELECT constraint_name FROM all_constraints "
                    + "WHERE table_name = UPPER(?) AND constraint_type = 'P')";
            Optional<ResultSet> rsPK = bbdd.select(sqlPK, nombreTabla);

            System.out.println("Clave primaria de " + nombreTabla + ":");
            if (rsPK.isPresent()) {
                while (rsPK.get().next()) {
                    System.out.println("- " + rsPK.get().getString("column_name"));
                }
            }

            // Obtener las claves ajenas que salen de la tabla
            String sqlFKOut = "SELECT a.column_name, a.constraint_name, c_pk.table_name r_table_name "
                    + "FROM all_cons_columns a "
                    + "JOIN all_constraints c ON a.owner = c.owner AND a.constraint_name = c.constraint_name "
                    + "JOIN all_constraints c_pk ON c.r_owner = c_pk.owner AND c.r_constraint_name = c_pk.constraint_name "
                    + "WHERE c.constraint_type = 'R' AND a.table_name = UPPER(?)";
            Optional<ResultSet> rsFKOut = bbdd.select(sqlFKOut, nombreTabla);

            System.out.println("\nClaves ajenas que salen de " + nombreTabla + ":");
            if (rsFKOut.isPresent()) {
                while (rsFKOut.get().next()) {
                    System.out.println("- " + rsFKOut.get().getString("column_name")
                            + " (referencia a " + rsFKOut.get().getString("r_table_name") + ")");
                }
            }

            // Obtener las claves ajenas que apuntan a la tabla
            String sqlFKIn = "SELECT a.table_name, a.column_name, a.constraint_name "
                    + "FROM all_cons_columns a "
                    + "JOIN all_constraints c ON a.owner = c.owner AND a.constraint_name = c.constraint_name "
                    + "WHERE c.constraint_type = 'R' AND c.r_constraint_name IN "
                    + "(SELECT constraint_name FROM all_constraints "
                    + "WHERE constraint_type IN ('P', 'U') AND table_name = UPPER(?))";
            Optional<ResultSet> rsFKIn = bbdd.select(sqlFKIn, nombreTabla);

            System.out.println("\nClaves ajenas que apuntan a " + nombreTabla + ":");
            if (rsFKIn.isPresent()) {
                while (rsFKIn.get().next()) {
                    System.out.println("- " + rsFKIn.get().getString("table_name") + "."
                            + rsFKIn.get().getString("column_name"));
                }
            }
        } catch (SQLException ex) {
            Logger.getLogger(Departamento.class.getName()).log(Level.SEVERE, null, ex);
